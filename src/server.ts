...
function serverInstructions(config: ServerConfig, toolNames: ToolNames): string {
  ...
  return `Use DevSpace as a local coding workspace. Call ${toolNames.openWorkspace} once per project folder or worktree to obtain a workspaceId. Reuse that same workspaceId for all later file, search, edit, write, show-changes, shell, and Git publication tools in that folder; do not call ${toolNames.openWorkspace} again unless switching folders/worktrees, changing checkout/worktree mode, the workspaceId is rejected as unknown, or the user explicitly asks to reopen. Close managed worktree sessions with ${toolNames.closeWorkspace}; it refuses dirty worktrees and removes clean worktrees through Git. When the user explicitly requests one fresh ChatGPT Web conversation with a supplied prompt, invoke web_launch; do not infer this trigger from text typed in another composer. WEB-LAUNCH-0 returns only launch acknowledgement and does not capture assistant output or continue the spawned conversation. When the user explicitly requests one bounded proof that a fresh ChatGPT Web conversation can discover and invoke DevSpace, invoke web_connector_proof. Do not invoke web_connector_probe directly unless the current launched conversation contains the exact one-time challenge supplied by WEB-CONNECTOR-1. ${agentsMd}${skills}${inspection}Batch known context reads with ${toolNames.readBatch} to reduce host approval prompts. Prefer ${toolNames.edit} for targeted modifications, ${toolNames.write} only for new files or complete rewrites, and ${toolNames.shell} for tests, builds, Git inspection, package scripts, and commands that are better executed by the shell. When the user explicitly requests a commit or push, use publish_git_changes with the existing workspaceId, a workingDirectory relative to the workspace root, and exact file paths. Do not construct /mnt paths, Windows absolute paths, or shell cd commands for workspace navigation; use the workingDirectory field. Do not use ${toolNames.shell} to edit tracked project files or ordinary working-tree contents, or to mutate the Git index, history, remotes, or branches. Use ${toolNames.edit} or ${toolNames.write} for project content. The shell may create explicitly requested repository-local operational state only when native create-only or atomic filesystem semantics are required, the target remains inside the opened workspace, and the state is excluded from Git. Do not write such state to machine-global locations. Avoid shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, generated scripts, or other commands whose purpose is to write project files.${showChanges}`;
}
...
export function createMcpServer(
  ...
  webConnectorProof: Pick<
    ...
    "proveConnectorAccess" | "acceptProbe"
  >,
): McpServer {
  ...
  registerAppTool(
    ...
    toolNames.shell,
    {
      ...
      description: config.minimalTools
        ? `Run a shell command inside an open workspace. Use for tests, builds, Git inspection, package scripts, search, file discovery, and directory inspection. In minimal tool mode, ${toolNames.grep}, ${toolNames.glob}, and ${toolNames.ls} are disabled; use command-line tools such as grep, rg, find, ls, and tree for those read-only inspection actions. Use publish_git_changes, not ${toolNames.shell}, when the user explicitly requests staging, commit, or push. Do not use ${toolNames.shell} to edit tracked project files or ordinary working-tree contents, or to mutate the Git index, history, remotes, or branches. Use ${toolNames.edit} for targeted project changes and ${toolNames.write} for new project files or full rewrites. The shell may create explicitly requested repository-local operational state only when native create-only or atomic filesystem semantics are required, the target remains inside the opened workspace, and the state is excluded from Git. Do not write such state to machine-global locations. Do not use shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, generated scripts, or other commands to write project files. Prefer ${toolNames.read} for direct file reads. Use the workingDirectory field instead of cd or translated absolute paths. Call open_workspace first and pass workspaceId. This is powerful local execution and should only be exposed behind strong authentication.`
        : `Run a shell command inside an open workspace. Use for tests, builds, Git inspection, package scripts, and commands that are better executed by the shell. Use publish_git_changes, not ${toolNames.shell}, when the user explicitly requests staging, commit, or push. Do not use ${toolNames.shell} to edit tracked project files or ordinary working-tree contents, or to mutate the Git index, history, remotes, or branches. Use ${toolNames.edit} for targeted project changes and ${toolNames.write} for new project files or full rewrites. The shell may create explicitly requested repository-local operational state only when native create-only or atomic filesystem semantics are required, the target remains inside the opened workspace, and the state is excluded from Git. Do not write such state to machine-global locations. Do not use shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, generated scripts, or other commands to write project files. Prefer ${toolNames.read}, ${toolNames.grep}, ${toolNames.glob}, and ${toolNames.ls} for file inspection. Use the workingDirectory field instead of cd or translated absolute paths. Call open_workspace first and pass workspaceId. This is powerful local execution and should only be exposed behind strong authentication.`,
      inputSchema: {
        workspaceId: z
          .string()
          .describe("Workspace identifier returned by open_workspace."),
        command: z
          .string()
          .describe(
            `Shell command to run for tests, builds, package scripts, and Git inspection. It must not edit tracked project files or ordinary working-tree contents, mutate the Git index, history, remotes, or branches, or write operational state outside the opened workspace. It may create explicitly requested repository-local operational state when native create-only or atomic filesystem semantics are required and the state is excluded from Git. Use publish_git_changes for explicitly authorized staging, commit, and push operations, and use ${toolNames.edit} or ${toolNames.write} for project content changes.`,
          ),
        workingDirectory: z
          .string()
          .optional()
